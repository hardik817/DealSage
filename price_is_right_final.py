...
#             def do_run():
#                 logging.info("Running agent framework...")
#                 new_opportunities = self.get_agent_framework().run()
#                 table = table_for(new_opportunities)
#                 return table

#             def run_once_with_logging(initial_log_data):
#                 log_queue = queue.Queue()
#                 result_queue = queue.Queue()
#                 setup_logging(log_queue)
...
#                 log_data = initial_log_data
#                 final_result = None
#                 start_time = time.time()

#                 while time.time() - start_time < 30:  # Timeout: 30s max wait
#                     try:
...
#                 yield log_data, html_for(log_data), final_result or []
...
#             # Initial load
#             ui.load(run_once_with_logging, inputs=[log_data], outputs=[log_data, logs, opportunities_dataframe])

#             # Timer trigger every 5 minutes
#             timer = gr.Timer(value=300, active=True)
#             timer.tick(run_once_with_logging, inputs=[log_data], outputs=[log_data, logs, opportunities_dataframe])
